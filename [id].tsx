...
//forecast info
    ...
    if(currentForecast){
        barData = currentForecast.windSpeedsKts.map((v, i) => {
            const windDir = currentForecast.windDirectionsDeg[i];
            const degTxt = degToDir(windDir);
            
            let barColor = 'black';
            if(isSafe(degTxt, idealWinds)){
                barColor = 'green'
            } else if(!isSafe(degTxt, idealWinds)){
                barColor = 'red'
            }
            ...
            <Pressable onPress={() => setSelectedForecast(forecastDay3)} >
                <View style={styles.card}>
                    <View style={styles.text}>
                        ...
                        <Text style={{fontSize: 17, fontWeight:'500'}}>Dir: {degToDir(forecastDay3.AvgWindDirection)}</Text>
                    </View>
